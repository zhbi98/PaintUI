...
#define VOLT_MIN 2700U /*mV*/
#define VOLT_MAX 4500U /*mV*/

#define DIV 4096U /*12Bit*/
#define VREF 280U /*2.8V, 2800mV, Magnify 100x*/
#define RP 57U /*R(Total):R(1)=5.7(M):1(M), Magnify 10x*/
 ...
 */
void nt_bat_set_ori_val(int16_t val)
{
 ...
 */
void nt_bat_move(int16_t data_p[], 
    uint32_t len)
{
    int16_t i = 0, j = 0;
    int16_t fval, bval;
...
void nt_bat_ocv_est_val()
{
    ...
    if (!liocv.end) nt_bat_set_ori_val(3000);
...
int32_t nt_bat_ocv_fit_soc()
{
    ...
    soc = (int32_t)((volt - VOLT_MIN) * 100.0 / 
