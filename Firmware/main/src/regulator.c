...
/**
 * Sets the value type for register configuration,
 * Updates the val_type field in the global regu structure.
 * @param _type The type value to set for register configuration
 */
...
/**
 * Get the value type for register configuration,
 * Get the val_type field in the global regu structure.
 */
...
/**
 * Retrieves and processes the changed value based on register type,
 * Sets regu.data as fallback value,Updates the register data field if not already set.
 */
static void regu_get_chg_val()
{
...
/**
 * Formats and validates register reference values for display
 */
static void regu_fmtval_refer()
{
  uint16_t _type = regut_val_ge_type();
...
/**
 * Gets the index of the first non-zero valid bit in a numeric string.
 * @param str_p Input string containing numeric characters (may include '.').
 * @return Index of first non-zero character (0-5), adjusted for edge cases.
 */
static uint8_t regu_valid_bit_get_idx(uint8_t * str_p)
{
  uint8_t _valid = 0;
...
void regu_LR_move(int32_t val)
{
  ...
  if (val == 1) { /*Right*/
    if (regu.cur_pos < regu.cur_movlim) {
      regu.cur_pos++;
      _powval = pow(10, regu.std_unit_len - regu.start_pos);
    ...
    for (_idx = len - 1; _idx > 0; _idx--) {
      ...
      if ((count % 3) == 0) {
        ...
        *str_p = '\0';
        ...
        sprintf(_sep, "%c", _char);
        if (_char >= ' ' && _char <= '~')
          strcat(regu.refer[1], _sep);
